...
class ArsMsfStateEstimator:
  ...
  def predict(self, timestamp):
    ...
    Fx[0:3, 0:3] = Identity
    # Position k+1 - Velocity linear k
    Fx[0:3, 4:7] = np.dot(Identity,delta_time)

    #estim_x_k1k_robot_posi -estim_x_k1k_robot_velo_lin_world
    ...
    #Fn[7:7, 4:4] =  estim_x_k1k_robot_velo_ang_world - cov_meas_velo_ang
    ...
    # Covariance
    # TODO BY STUDENT----------------------------------------------------------------------
    # Transpose in a different way 
    transpose_Fx= np.transpose(Fx)
    transpose_Fn = np.transpose(Fn)
    Qn = np.zeros((4,4), dtype=float)
    # Diagonalize cov_meas velo)lin 3*3
    np.fill_diagonal(Qn,val=self.cov_meas_velo_lin)
    Qn[3][3]= self.cov_meas_velo_ang
    estim_P_k1k = np.add(np.dot(np.dot(Fx, estim_P_kk),transpose_Fx), np.dot(np.dot(Fn,Qn),transpose_Fn))
    
    print("Calculated here",estim_P_k1k)
  ...
  def update(self):
    ...
    if(flag_set_meas_robot_posi == True):
      ...
      # Innovation of the measurement
      # TODO BY STUDENT: Done
      innov_meas_robot_posi = pred_z_robot_posi- self.meas_robot_posi
    ...
    if(flag_set_meas_robot_vel_robot == True):
      # Predicted measurement
      # TODO BY STUDENT: Done
      pred_z_robot_velo_lin_robot = estim_x_k1k_robot_velo_lin_world
    ...
    Rz = np.zeros((dim_meas,8), dtype=float)
    Hx_idx = 0
    if(flag_set_meas_robot_posi == True):
      # Meas robot posi - robot posi
      # TODO BY STUDENT: Done
      Hx[Hx_idx:Hx_idx+3, 0:3] = np.identity(3)
      Rz[Hx_idx:Hx_idx+3, 0:3] = np.identity(3)
      Hx_idx += 3

    if(flag_set_meas_robot_atti == True):
      # Meas robot atti - robot atti
      Hx[Hx_idx:Hx_idx+1, 3] = 1.0
      Rz[Hx_idx:Hx_idx+1, 3] = 1.0
      Hx_idx += 1

    if(flag_set_meas_robot_vel_robot == True):
      # Meas velo lin - robot atti
      # TODO BY STUDENT: HERE R(w,r)

      # Step 1: Find R 
      #Make sure the angle is in Radian
      Rz[Hx_idx:Hx_idx+3, 4:7] = np.identity(3)
      Rz[Hx_idx:Hx_idx+1, 7] = 1.0
      phi = innov_meas_robot_atti_angle
      R= estim_x_k1k_robot_atti_rot_mat
      Rt=R.transpose()

      v= self.estim_robot_velo_lin_world
      Hx[Hx_idx:Hx_idx+3, 3]= np.dot(Rt,v)
      # Meas velo lin - robot velo lin
      # TODO BY STUDENT: HERE 
      Hx[Hx_idx:Hx_idx+3, 4:7] = np.dot(R,phi)
      Hx_idx += 3
    ...
    # TODO BY STUDENT : DONE
    Ht=Hx.transpose()
    Rz=np.zeros((dim_meas,dim_meas), dtype=float)
    Rz=np.identity(dim_meas)
    print("RZ",np.shape(Rz),Rz)
    print("HT",np.shape(Ht),Ht)
    print("estim",np.shape(estim_P_k1k),estim_P_k1k)
    print("Hx",np.shape(Hx),Hx)
    cov_innov_meas = np.add(np.dot((np.dot(Hx,estim_P_k1k)),Ht),Rz)

  
    # Kalman Gain
    # TODO BY STUDENT : DONE
    print("Cov_meas",cov_innov_meas)
    print("Det",np.linalg.det(cov_innov_meas))
    if(np.linalg.det(cov_innov_meas)!= 0.0):
        St=np.linalg.inv(cov_innov_meas)
    else:
        St= cov_innov_meas
    kalman_gain = np.dot(np.dot(estim_P_k1k, Ht),St)
    
    #Question: Angle and phi what are they? how to initialize Rz??/??
    ...
    # TODO BY STUDENT 
    I=np.identity(8)
    estim_P_k1k1 = np.dot(np.subtract(I, np.dot(kalman_gain,Hx)),estim_P_k1k)
